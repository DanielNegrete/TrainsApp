...
public class UserController {
	...
	public User saveUser(@Valid @RequestBody UserRequestDTO userRequestDTO) {
		logger.debug("User: " + userRequestDTO.getUsername() + " got saved");
		return userService.saveUsers(userRequestDTO);
	}
    
    @GetMapping("/user/{userId}")
    public Optional<User> validateUser(@NotNull @Min(1) @PathVariable("userId") Integer userId) {
    ...
    public List<Train> getTrainsBySourceDestinationDate(@Valid @RequestBody TrainRequestDTO trainRequestDTO){
    	return userService.getJourneys(trainRequestDTO);
    }
