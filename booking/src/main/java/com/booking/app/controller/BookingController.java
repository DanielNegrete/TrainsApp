...
public class BookingController {
	...
	public List<Booking> getBookingsByUserId(@NotNull @Min(1) @PathVariable("userId") Integer userId){
		logger.info("Getting all the booking of the user: " + userId);
		return bookingService.getBookingByUserId(userId);
	}

	@PostMapping("/savebooking")
	public ResponseEntity<String> saveBooking(@Valid @RequestBody BookingRequestDTO bookingRequestDTO){
	...
	public Optional<User> getUserByUserId(@NotNull @Min(1) @PathVariable("userId") Integer userId){
	...
	@GetMapping("/{trainId}")
	public Optional<Train> getTrainsByTrainId(@NotNull @Min(1) @PathVariable("trainId") Integer trainId){
